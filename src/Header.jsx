    ...
    <div className="md:px-16">
      ...
      {isOpen && (
        <div className="modal absolute w-full h-[100vh] top-0 px-6 bg-blue-950/95 pt-7 z-10">
          <div className="flex justify-between items-center pb-10 border-b border-b-gray-400">
            <svg
          ...
          <div>
            {menu.map((item, i) => (
              <p
                className={`text-white text-[1.3rem] py-5 text-center border-b border-b-gray-400 cursor-pointer`}
                key={i}
              >
                {item}
              </p>
          ...
          <div className="py-8">
            <Button variant="login" className="w-full py-3">
              LOGIN
            </Button>
          </div>
          <div className="flex w-full flex-row gap-8 items-center justify-center mt-4 pb-3 absolute bottom-[2rem] left-0 right-0">
            {socialLinks.map((item, i) => (
              <img key={i} className="cursor-pointer" src={item.src} />
            ))}
          </div>
        </div>
      )}
